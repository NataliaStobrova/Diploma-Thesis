...
posun_doch_vek_m = 0; %= input('Zadajte posun veku odchodu do dochodku pre mužov (v mesiacoch): ');
posun_doch_vek_z = 0; %= input('Zadajte posun veku odchodu do dochodku pre ženy (v mesiacoch): ');
limit_2_pilier = 35; %= input('Zadajte vek, do kedy môže poistenec vstúpiť do 2. piliera: ');
...
valorizacia = 0.026; %= input('Zadajte ročnú mieru valorizácie dôchodkov: '); %za rok 2020  
valorizacia2 = 0.02;
narast_mzdy = 0.0562; %= input('Zadajte ročnú mieru narastu mzdy: '); %priemer za roky 2017 - 2020 indexu nominalnej mzdy
narast_mzdy2 = 0.05;
priemerna_mzda_mes = 1133; %= input('Zadajte priemernu mesačnú mzdu: '); %priemerna mzda za 06/2020
priemerny_dochodok_mes = 484.94; %= input('Zadajte priemerny mesačný dôchodok: '); %priemerny dochodok za 30.06.2020
zlom = dochodkovy_vek_m0 - vstup_na_trh_prace; %rok prechodu z aktualneho na novy system
pomer_zakl_dod = 0.7; %cast z predoslej davky (sucastneho stavu) 
...
for k=1:predpoved-zlom
 ...
 while ((narast_nov_dodat(k) > 0.01) && (deficit(k) > 1.5))  %pokym deficit > 2 alebo narast_noc_dodat >0.01, ked deficit = 1% hdp, tak doratame ake musi byt narast_nov_dodat(i)
  ...
  [celk_m, celk_z, doSP] = defi(narast_novy_dodat, k, predpoved, posun_doch_vek_m, posun_doch_vek_z, valorizacia, narast_mzdy2, valorizacia2,...
  narast_mzdy, vstup_na_trh_prace, dochodkovy_vek_m0, dochodkovy_vek_z0, limit_2_pilier, miera_nezamestnanosti_m, miera_nezamestnanosti_z, priemerna_mzda_mes, priemerny_dochodok_mes, pomer_zakl_dod, hdp);
...
%Posun Vekovej struktury kvoli poctu vyplacanych dochodkov
 ...
 if (j < zlom) %po zlom. roku sa budu vyplacat uz jednotne davky (bez kratenia dochodku z 1. piliera pre druhopilieristov)
  ...
  if ((j > zlom) & (dv_m(j) == dv_m(j-1)))
   jednotna_davka_mes(j-zlom+1) = jednotna_davka_mes(j-zlom) + zaokruh(jednotna_davka_mes(j-zlom)*narast_mzdy2); 
  ...
  for i=dv_m(j)-dv_m(1)+2:dv_m(j)-dv_m(1)+1+j-zlom 
   if (i > dv_m(j) - dv_m(1) + 1 & i <= 99 - dv_m(1) + 1 )
    ...
    %Zaokruhlovacia funkcia zaokruh
     dochodok_mes_m(i,j) = dochodok_mes_m(i-1,j-1) + zaokruh(dochodok_mes_m(i-1,j-1)*valorizacia2); 
     dodatok_len1_pilier_mes_m(i,j) = dodatok_len1_pilier_mes_m(i-1,j-1) + zaokruh(dodatok_len1_pilier_mes_m(i-1,j-1)*dodatok(j-zlom)); %0.01;
     dochodok_roc_m(i,j) = dochodok_mes_m(i,j) * 12;
     dodatok_len1_pilier_roc_m(i,j) = dodatok_len1_pilier_mes_m(i,j) * 12;
    end
   end
  ...
  for i=dv_m(j)-dv_m(1)+1+j-zlom+1:99-dv_m(1)+1
   ...
   elseif (dochodok_len1_pilier_mes_m(i-1,j-1) < 361.6)  
    ...
    %Zaokruhlovacia funkcia zaokruh
    dochodok_len1_pilier_mes_m(i,j) = dochodok_len1_pilier_mes_m(i-1,j-1) + zaokruh(dochodok_len1_pilier_mes_m(i-1,j-1)*valorizacia2); 
    dochodok_2_pilier_mes_m(i,j) = dochodok_2_pilier_mes_m(i-1,j-1) + zaokruh(dochodok_2_pilier_mes_m(i-1,j-1)*valorizacia2);
    dochodok_len1_pilier_roc_m(i,j) = dochodok_len1_pilier_mes_m(i,j) * 12;
    dochodok_2_pilier_roc_m(i,j) = dochodok_2_pilier_mes_m(i,j) * 12;
   end   
   if (i > dv_m(j) - dv_m(1) + 1 & dv_m(1) - vstup_na_trh_prace + 1 + (i - 1) <= length(spor2_m)) %pre i > 1 sa valorizuju priznane dochodky
...
%Posun Vekovej struktury kvoli poctu vyplacanych dochodkov
 ...
 if (j < zlom) %po zlom. roku sa budu vyplacat uz jednotne davky (bez kratenia dochodku z 1. piliera pre druhopilieristov)
  ...
  if (j == zlom)
   jednotna_davka_mes(j-zlom+1) = pomer_zakl_dod * adh(zlom-1) * (1 + narast_mzdy) * pocet_rokov_len1_pilier_z_predpoved(j,3);      %v 1. roku prognozy  
   jednotna_davka_roc(j-zlom+1) = jednotna_davka_mes(1) * 12; %v 1. roku prognozy
   dodatok_mes(j-zlom+1) = adh(zlom-1) * (1 + narast_mzdy) * pocet_rokov_len1_pilier_z_predpoved(j,3) - jednotna_davka_mes(j-zlom+1);  %v 1. roku prognozy
   dodatok_roc(j-zlom+1) = dodatok_mes(j-zlom+1) * 12; %v 1. roku prognozy
  ...
  for i=dv_z(j)-dv_z(1)+2:dv_z(j)-dv_z(1)+1+j-zlom 
   if (i > dv_z(j) - dv_z(1) + 1 & i <= 99 - dv_z(1) + 1)
    ...
    elseif (dodatok_len1_pilier_mes_z(i-1,j-1) < 361.6)  
     ...
     %Zaokruhlovacia funkcia zaokruh
     dochodok_mes_z(i,j) = dochodok_mes_z(i-1,j-1) + zaokruh(dochodok_mes_z(i-1,j-1)*valorizacia2); 
  ...
  for i=dv_z(j)-dv_z(1)+1+j-zlom+1:99-dv_z(1)+1
   ...
   %Zaokruhlovacia funkcia zaokruh
    dochodok_len1_pilier_mes_z(i,j) = dochodok_len1_pilier_mes_z(i-1,j-1) + zaokruh(dochodok_len1_pilier_mes_z(i-1,j-1)*valorizacia2); 
    dochodok_2_pilier_mes_z(i,j) = dochodok_2_pilier_mes_z(i-1,j-1) + zaokruh(dochodok_2_pilier_mes_z(i-1,j-1)*valorizacia2);
    dochodok_len1_pilier_roc_z(i,j) = dochodok_len1_pilier_mes_z(i,j) * 12;
    dochodok_2_pilier_roc_z(i,j) = dochodok_2_pilier_mes_z(i,j) * 12;
   end   
   if (i > dv_z(j) - dv_z(1) + 1 & dv_z(1) - vstup_na_trh_prace + 1 + (i - 1) <= length(spor2_z)) %pre i > 1 sa valorizuju priznane dochodky
...
% hold on
% scatter(pocet_rokov_2_pilier_m_predpoved(1:end,1), vyplacanie_doch_m + vyplacanie_doch_z, 25, 'filled', 'MarkerFaceColor', [0.7 0.4 1]) %[0.8500 0.3250 0.0980]) %[0.7 0.4 1] pre posun_doch_vek_m/z = 2 
% title({'Príjmy a výdavky Sociálnej poisťovne'})
% xlabel('Rok prognózy')
% ylabel('Príjmy / Výdavky Sociálnej poisťovne')
% ylim([0 200000000000])
% legend({'príjmy (M1 a M2)','výdavky (M1)', 'výdavky (M2)'},'Location','northwest') 
% legend({'príjmy (posun\_doch\_vek_{m/z} = 0)','výdavky (posun\_doch\_vek_{m/z} = 0) / (M1)', 'výdavky (posun\_doch\_vek_{m/z} = 0) / (M2)','príjmy (posun\_doch\_vek_{m/z} = 2)','výdavky (posun\_doch\_vek_{m/z} = 2) / (M1)', 'výdavky (posun\_doch\_vek_{m/z} = 2) / (M2)'},'Location','northwest') 
% legend({'príjmy (posun\_doch\_vek_{m/z} = 0)','výdavky (posun\_doch\_vek_{m/z} = 0)','výdavky (posun\_doch\_vek_{m/z} = 0) / (M1)', 'výdavky (posun\_doch\_vek_{m/z} = 0) / (M2)','príjmy (posun\_doch\_vek_{m/z} = 2)','výdavky (posun\_doch\_vek_{m/z} = 2)', 'výdavky (posun\_doch\_vek_{m/z} = 2) / (M1)', 'výdavky (posun\_doch\_vek_{m/z} = 2) / (M2)'},'Location','northwest')  
...
% title({'Počet prispievateľov vs. počet poberateľov'})
% xlabel('Rok prognózy')
% ylabel('Počet prispievateľov / počet poberateľov')
% ylim([500000 2000000])
% legend({'počet prispievateľov', 'počet poberateľov'},'Location','northeast')
...
% figure
% scatter(pocet_rokov_2_pilier_m_predpoved(1:end,1), deficit, 25, 'filled', 'MarkerFaceColor', [0.7 0.4 1]) %[0.8500 0.3250 0.0980]) [0.7 0.4 1] pre posun_doch_vek_m/z = 2 
% title({'Deficit starobného fondu Sociálnej poisťovne [% HDP]'})
% xlabel('Rok prognózy')
% ylabel('Deficit starobného fondu ')
% ylim([0 5])
% legend({'deficit (M1)', 'deficit (M2)'},'Location','northeast')
% legend({'deficit (posun\_doch\_vek_{m/z} = 0) / (M1)', 'deficit (posun\_doch\_vek_{m/z} = 0) / (M2)', 'deficit (posun\_doch\_vek_{m/z} = 2) / (M1)', 'deficit (posun\_doch\_vek_{m/z} = 2) / (M2)'},'Location','southwest')
% legend({'deficit (posun\_doch\_vek_{m/z} = 0)','deficit (posun\_doch\_vek_{m/z} = 0) / (M1)', 'deficit (posun\_doch\_vek_{m/z} = 0) / (M2)', 'deficit (posun\_doch\_vek_{m/z} = 2)', 'deficit (posun\_doch\_vek_{m/z} = 2) / (M1)', 'deficit (posun\_doch\_vek_{m/z} = 2) / (M2)'},'Location','southwest', 'FontSize', 7)
